...
def boar_brawl(player_score, opponent_score):
    ...
    return max(1, 3 * abs(player_ones_digit - opponent_tens_digit))
    # END PROBLEM 2
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    average_score = make_averaged(sus_update, 10)
    s = 0
    max_rolls = 0
    for i in range(11):
        sus_score = average_score(i, score, opponent_score)   # six_sided dice by default
        if sus_score > 100:   # goal = 100 by default
            return i
        if sus_score > s:
            s = sus_score
            max_rolls = i
